...
import { Settings2, Play, Square, Sparkles, Cpu, Database, Music, Layers, Eye, BookOpen, User } from 'lucide-react';
...
const Sidebar: React.FC<SidebarProps> = ({
  preferences, setPreferences, isGenerating, isSolving, isAutoMode, pipelineStep, isFullPackage, currentPackageIndex, packageQueueLength, onToggleFullPackage, hasImage, onGenerate, onAutoMode, onToggleSolve, musicTracks, selectedTrackId, onAddMusicTracks, onAddCloudTrack, onSelectTrack, onRemoveTrack, onGifChange, onChannelLogoChange
}) => {
  return (
    <aside className="w-[400px] flex flex-col h-full bg-[#050508] border-r border-white/5 text-slate-300 font-sans z-40 overflow-hidden shadow-[10px_0_30px_rgba(0,0,0,0.5)]">
      ...
      <div className="shrink-0 px-4 py-3 bg-zinc-950/50 border-b border-white/5 backdrop-blur-md flex items-center justify-between">
        <div className="flex items-center gap-2.5">
          ...
          <span className="text-[10px] font-black uppercase tracking-[0.2em] text-zinc-400">Studio_Matrix</span>
        </div>
        <div className="flex items-center gap-2">
          <div className={`w-1 h-1 rounded-full ${isGenerating || isSolving ? 'bg-blue-500 animate-pulse' : 'bg-zinc-700'}`} />
        ...
        <section className="space-y-4 pt-2 border-t border-white/5">
            ...
            <div className="space-y-3">
                ...
                <div className="flex items-center justify-between p-3 bg-blue-900/5 border border-blue-500/10 rounded-xl">
                    <div className="flex items-center gap-2.5">
                        ...
                        <div className="flex flex-col">
                            <span className="text-[10px] font-bold text-blue-400 uppercase tracking-wide">Documentary Mode</span>
                            <span className="text-[8px] text-zinc-500 font-mono tracking-wider">AI Factual Overlays</span>
                        </div>
                    </div>
                    <label className="relative flex items-center cursor-pointer">
                        <input 
                            ...
                            onChange={() => setPreferences({ ...preferences, showDocumentaryTips: !preferences.showDocumentaryTips })}
                ...
                <VisionInput 
                    ...
                    onChange={(subject, topicType, topicCategory) => setPreferences({ 
                        ...preferences, 
                        subject, 
