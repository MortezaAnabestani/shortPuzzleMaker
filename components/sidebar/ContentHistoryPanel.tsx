...
const ContentHistoryPanel: React.FC = () => {
  ...
  const handleClear = () => {
    if (confirm(`⚠️ آیا مطمئن هستید که می‌خواهید ${historyCount} رکورد را پاک کنید؟\nاین عمل قابل بازگشت نیست!`)) {
      clearContentHistory();
      setHistoryCount(0);
  ...
  return (
    <div className="bg-white/5 backdrop-blur-sm rounded-lg p-4 border border-white/10">
      <div className="flex items-center justify-between mb-3">
        ...
        <div className="bg-blue-500/20 px-2 py-0.5 rounded text-xs text-blue-300">
          {historyCount} رکورد
        </div>
      </div>
      ...
      <div className="space-y-2">
        ...
        >
          <span className="text-xs text-purple-300">
            {showRecent ? 'پنهان کردن' : 'نمایش'} 10 آخرین
          </span>
          ...
          <div className="space-y-2 max-h-48 overflow-y-auto custom-scrollbar">
            {recentRecords.map((record) => (
              ...
              >
                ...
                <div className="text-xs text-gray-300 leading-tight line-clamp-2">
                  {record.coreSubject}
                </div>
