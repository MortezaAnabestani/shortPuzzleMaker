  ...
  return (
    <section className="space-y-4 font-sans">
      ...
      <div className="flex justify-between items-end border-b border-slate-800 pb-2">
        <SectionLabel icon={<Music className="w-3.5 h-3.5" />}>Music Management</SectionLabel>
        <button
              ...
              {tracks.map((track) => {
                ...
                return (
                  ...
                  >
                    ...
                    >
                      <div
                        className={`shrink-0 transition-colors ${isSelected ? "text-[#007acc]" : "text-slate-600 group-hover:text-slate-500"}`}
                      >
                        {isSelected ? (
                      ...
                      <div className="flex items-center gap-2 overflow-hidden w-full">
                        ...
                        <span
                          className={`text-[11px] font-medium truncate ${isSelected ? "text-blue-100" : "text-slate-400 group-hover:text-slate-300"}`}
                        >
                          {track.name}
                        </span>
                      </div>
        ...
        {selectedTrackId && (
          <div className="flex items-start gap-2 px-3 py-2 bg-[#007acc]/5 border border-[#007acc]/20 rounded-md">
            ...
            <p className="text-[10px] text-blue-400/90 font-medium leading-relaxed">
              <span className="font-bold text-[#007acc]">LOCKED:</span> فایل صوتی انتخاب شده به عنوان منبع
              اصلی برای تمام خروجی‌ها استفاده خواهد شد.
            </p>
          </div>
        )}
      </div>
