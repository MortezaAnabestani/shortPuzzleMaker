    ...
    <div className="w-full flex flex-col gap-4">
      ...
      <div className="grid grid-cols-1 gap-2">
        {SOUND_SLOTS.map((slot) => {
          ...
          return (
            <div key={slot.type} className="group relative flex items-center justify-between bg-zinc-900/40 border border-white/5 p-2 rounded-xl hover:border-blue-500/20 transition-all">
              <input 
                type="file" 
                // Fix: ref callback must return void. Map.set returns the Map, causing a TS error.
                ref={el => { if (el) fileInputs.current.set(slot.type, el); }}
              ...
              <div className="flex items-center gap-3 overflow-hidden">
                <div className={`w-8 h-8 rounded-lg flex items-center justify-center transition-all ${isLoaded ? 'bg-blue-600 text-white shadow-lg' : 'bg-zinc-800 text-zinc-600'}`}>
                  {slot.icon}
                </div>
