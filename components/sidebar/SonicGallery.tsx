        ...
        <div className="flex-1 overflow-y-auto custom-scrollbar bg-slate-900">
          {SONIC_LIBRARY.map((track, index) => {
            ...
            return (
              <div
                key={track.id}
                className={`group grid grid-cols-12 gap-2 px-4 py-2.5 items-center border-b border-slate-800/50 hover:bg-slate-800/40 transition-all duration-200 ${
                  isActive ? "bg-[#007acc]/5" : ""
                }`}
              >
                {/* Play Control */}
                <div className="col-span-1 flex justify-center">
                  <button
                    ...
                    className={`w-8 h-8 rounded-md flex items-center justify-center border transition-all ${
                      isPlaying
                        ? "bg-[#007acc] border-[#007acc] text-white shadow-[0_0_10px_rgba(0,122,204,0.3)]"
                ...
                <div className="col-span-6 sm:col-span-5 flex flex-col justify-center pl-2">
                  <div className="flex items-center gap-2">
                    <span
                      className={`text-[12px] font-medium truncate ${
                        isActive ? "text-[#007acc]" : "text-slate-300 group-hover:text-white"
                      }`}
                    >
                      {track.title}
                    </span>
                  ...
                  <div className="flex items-center gap-2 mt-0.5">
                    ...
                    <span className="hidden sm:inline-flex px-1.5 py-0.5 rounded-[3px] bg-slate-800 text-[9px] text-slate-400 border border-slate-700/50">
                      MP3
                    </span>
                  </div>
                </div>
                ...
                <div className="col-span-5 sm:col-span-3 flex justify-end">
                  <button
                    ...
                    className={`h-8 px-3 rounded-md text-[10px] font-bold tracking-wide border transition-all flex items-center gap-2 uppercase ${
                      isActive
