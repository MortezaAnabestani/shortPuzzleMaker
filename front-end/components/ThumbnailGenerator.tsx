...
const getLensColor = (lens?: NarrativeLens): string => {
  switch (lens) {
    case NarrativeLens.HIDDEN_DISCOVERY:
      return "#FFD700"; // Gold - discovery/treasure
    case NarrativeLens.WHY_MYSTERY:
      return "#9370DB"; // Purple - curiosity/mystery
    case NarrativeLens.COMPARISON:
      return "#20B2AA"; // Teal - balance/comparison
    case NarrativeLens.UNSOLVED_ENIGMA:
      return "#FF6B9D"; // Rose - enigma/unknown
    case NarrativeLens.TRANSFORMATION:
      return "#00CED1"; // Turquoise - change/evolution
    case NarrativeLens.COUNTDOWN:
      return "#FF4500"; // Orange-Red - urgency/countdown
    case NarrativeLens.ORIGIN_STORY:
      return "#4169E1"; // Royal Blue - heritage/origin
    default:
      return "#ff3333"; // Classic red - default
  }
};
  ...
  if (isLoading)
    return (
      <div className="mt-8 max-w-4xl mx-auto aspect-[9/19] max-h-[700px] bg-white/5 rounded-[4.5rem] border border-white/10 animate-pulse flex flex-col items-center justify-center">
        ...
        <div className="text-[10px] font-black text-slate-500 uppercase tracking-[0.6em]">
          Rendering 9:19 Vertical Asset...
        </div>
      </div>
    );
      ...
      <div className="bg-[#050505] border border-white/5 p-14 rounded-[4.5rem] shadow-2xl relative overflow-hidden">
        ...
        <div className="flex items-center justify-between mb-12 relative z-10">
          <div className="flex items-center gap-8">
            ...
            <div>
              <h3 className="text-2xl font-black text-white uppercase tracking-tighter">9:19 Studio Cover</h3>
              <p className="text-[11px] text-slate-600 font-black uppercase tracking-[0.5em] mt-1.5 italic">
                Retina Ready Vertical
              </p>
            </div>
          </div>
        ...
        <div
          className={`relative ${
            isShortsMode ? "max-w-[400px] mx-auto aspect-[9/19]" : "aspect-video"
          } rounded-[4rem] overflow-hidden border border-white/10 shadow-[0_0_120px_rgba(0,0,0,0.9)] group cursor-pointer bg-black`}
        >
