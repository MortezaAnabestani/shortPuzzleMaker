    ...
    <div className="w-full border border-slate-200 bg-white rounded-md overflow-hidden mb-4">
            ...
            >
              <div className="flex items-center gap-3">
                ...
                <div className="flex flex-col">
                  ...
                  <span className="text-[10px] font-mono text-slate-400 mt-1">ID: {s.id}</span>
                </div>
              </div>
              ...
              <div className="flex items-center">
                {isActive && (
                  <span className="text-[10px] font-mono text-[#007acc] animate-pulse">PROCESSING...</span>
                )}
                {isComplete && <span className="text-[10px] font-mono text-emerald-600">[DONE]</span>}
                {!isActive && !isComplete && (
                  <span className="text-[10px] font-mono text-slate-300">WAITING</span>
                )}
              </div>
            </div>
          );
      ...
      <div className="px-3 py-1.5 bg-slate-50 border-t border-slate-200 flex justify-between items-center">
        <span className="text-[10px] text-slate-400 font-mono">
          SYS_MODE: {isAutoMode ? "AUTO_SEQUENCE" : "MANUAL_OVERRIDE"}
        </span>
